        ...
        <div class="row m-0">
          ...
          <div class="col-lg-8 col-md-12 mob-p-0 mob-pt-10">
            ...
            <p class="date font-weight-bold text-muted mb-0 mt-0"> <i class="fas fa-user mr-1"></i> 
              <a href="{{url('author').'/'.str_replace(' ','-',preg_replace('/[^\p{L}\p{Z}\p{N}\p{M}]/u','', $ala->authorname)).'-'.$ala->author_id}}"> @if($ala->authorname != 'Samachar4media Bureau' || !isset($ala->authorname)){{$ala->authorname}}@else समाचार4मीडिया ब्यूरो ।।@endif</a> <i class="far fa-clock ml-2 mr-1"></i> <a href="#">{{\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $ala->pickdate)->diffForHumans()}} </a>
            </p>
            <p class="mt-3">
