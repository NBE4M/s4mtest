...
class PaginateRoute
{
     ...
     * @return string
     */
    public function renderPageList(LengthAwarePaginator $paginator, $full = true, $class = 'null', $additionalLinks = true)
    {
        ...
        $listItems = '<ul class="pagination pagination-sm cat-pg" role="navigation">';
        ...
        foreach (array_slice($urls, 0, 8) as $i => $url) {
            $pageNum = $i + 1;
            $css = '';
            ...
            $listItems .= "<li class='page-item $css'><a class='page-link' href=\"{$url}\">{$pageNum}</a></li>";
            
            
        }

        if ($this->hasNextPage($paginator) && $additionalLinks) {
             $listItems .= "<li class='page-item disabled' aria-disabled='true'><span class='page-link'>.....</span></li>";
